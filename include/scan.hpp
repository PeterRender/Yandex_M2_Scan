...
std::expected<details::scan_result<Ts...>, details::scan_error> scan(std::string_view input, std::string_view format) {
    // 1. Извлекаем из форматной и входной строки массив спецификаторов форматов и массив строковых значений, соответствующие плейсхолдерам
    auto parse_result = details::parse_sources(input, format);
    if (!parse_result) {
        return std::unexpected(parse_result.error());
    }
    // Кортеж с извлеченными массивами спецификаторов форматов и строковых значений
    auto [format_parts, input_parts] = *parse_result;
    ...
    // == Тестовая реализация (для одного параметра в пакете типа string) ==    
    
    // Проверяем на этапе компиляции, что в пакете один параметр
    if constexpr (expected_parts == 1) {
        ...
        // Если тип - строка, то обрабатываем
        if constexpr (std::same_as<std::remove_cv_t<FirstType>, std::string>) {
            // Парсим первую пару (спецификатор формата, входные данные)
            auto parse_result = details::parse_value_with_format<std::string>(input_parts[0], format_parts[0]);
            // Если ошибка парсинга, то возвращаем ее значение
            if (!parse_result) {
                return std::unexpected(parse_result.error());
            }
            // Успешный парсинг - создаем результат.
            return details::scan_result<Ts...>(std::make_tuple(parse_result.value()));
        }
    }
